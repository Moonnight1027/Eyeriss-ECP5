// Modified for Eyeriss_ECP5 (2026): send a zero-length packet (ZLP) when a
// bulk IN transfer ends on a full 32-byte packet, so that hosts such as
// Windows usbser.sys complete the read. See PipelineInState_Zlp.
// Original: https://github.com/davidthings/tinyfpga_bx_usbserial (Apache-2.0)
/*
    usb_uart_bridge_ep
...
module usb_uart_bridge_ep (
  ...
  localparam PipelineInState_Zlp       = 4;

  // Bulk IN transfers that end on a full packet must be terminated with a
  // zero-length packet, otherwise hosts such as Windows usbser.sys keep
  // waiting for more data. Count the bytes put into the current packet and
  // remember when the last one filled it.
  localparam InPacketSize = 32;          // MAX_IN_PACKET_SIZE of usb_fs_in_pe
  reg [5:0] in_put_cnt;
  reg       zlp_pending;
  ...
  always @(posedge clk) begin
      if ( reset ) begin
          in_put_cnt  <= 0;
          zlp_pending <= 0;
      end else if ( in_ep_data_put ) begin
          if ( in_put_cnt == InPacketSize - 1 ) begin
              in_put_cnt  <= 0;
              zlp_pending <= 1;
          end else begin
              in_put_cnt  <= in_put_cnt + 1;
              zlp_pending <= 0;
          end
      end else begin
          // a short packet was closed
          if ( in_ep_data_done_reg ) in_put_cnt <= 0;
          // the ZLP is being issued (see PipelineInState_Zlp). A done pulse
          // from the WaitData timeout right after a full packet is ignored by
          // the endpoint, so it must not clear the flag.
          if ( pipeline_in_state == PipelineInState_Zlp && !uart_in_valid &&
               in_ep_grant && in_ep_data_free )
              zlp_pending <= 0;
      end
  end
      ...
      if ( reset ) begin
          ...
          case( pipeline_in_state )
              PipelineInState_Idle: begin
                  ...
                  end else if ( zlp_pending && !uart_in_valid ) begin
                      // the stream ended on a full packet: after a short pause, send a ZLP
                      in_ep_timeout <= in_ep_timeout + 1;
                      if ( in_ep_timeout[ TimeoutWidth ] ) begin
                          in_ep_timeout <= 0;
                          in_ep_req_reg <= 1;
                          pipeline_in_state <= PipelineInState_Zlp;
                      end
                  end else begin
                      in_ep_timeout <= 0;
                  end
              end
              PipelineInState_Zlp: begin
                  if ( uart_in_valid ) begin
                      // more data after all: no ZLP needed, go put it
                      in_ep_req_reg <= 0;
                      pipeline_in_state <= PipelineInState_Idle;
                  end else if ( in_ep_grant && in_ep_data_free ) begin
                      // empty buffer + done = zero-length packet
